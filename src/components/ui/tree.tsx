              ...
              <AccordionPrimitive.Root type="multiple" defaultValue={expandedItemIds}>
                <AccordionPrimitive.Item value={item.id}>
                  ...
                  >
                    {item.icon &&
                      <item.icon
                        className="w-4 h-4 mr-2 shrink-0 text-accent-foreground/50"
                        aria-hidden="true"
                      />
                    }
...
>(({ className, item, isSelected, Icon, ...props }, ref) => {
  return (
    <div
      ...
      {...props}
    >
      {item.icon && <item.icon className="w-4 h-4 mr-2 shrink-0 text-accent-foreground/50" aria-hidden="true" />}
      {!item.icon && Icon && <Icon className="w-4 h-4 mr-2 shrink-0 text-accent-foreground/50" aria-hidden="true" />}
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header>
    ...
    >
      {children}
      <ChevronRight className="w-4 h-4 ml-auto transition-transform duration-200 shrink-0 text-accent-foreground/50" />
    </AccordionPrimitive.Trigger>
  </AccordionPrimitive.Header>
));
